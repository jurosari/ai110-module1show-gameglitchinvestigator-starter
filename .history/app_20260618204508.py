...
def reset_game(difficulty):
    """Reset all game state for a fresh game at the given difficulty.

    Single source of truth for what 'a new game' means, so the
    'New Game' button and the difficulty-change reset can't drift apart.
    """
...
if new_game:
    # ===== HUMAN VERSION (my original) — kept for reference =====
    # """
    ...
    # ===== AI VERSION (refactored) — reuses reset_game() helper =====
    reset_game(st.session_state.last_difficulty)
...
if submit:
    ...
    if not ok:
        ...
        #"""
        secret = st.session_state.secret #added
